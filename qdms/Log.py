...
def save_memristor_simulation_hdf5(memristor_sim, path):
    ...
    with h5py.File(f'{path}\\{filename}.hdf5', 'w') as f:
        ...
        pandas.DataFrame(memristor_sim.voltages_memristor).to_hdf(f'{path}\\voltages_memristor.hdf5', 'voltages_memristor')
...
def load_memristor_simulation_hdf5(path, circuit):
    ...
    with h5py.File(f'{path}', 'r') as file:
        ...
        timer = time.time()
        voltages_memristor = pandas.DataFrame(file.get("voltages_memristor")).to_dict('list')
